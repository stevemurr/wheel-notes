...
.tippy-box[data-animation=fade][data-state=hidden] {
  ...
  var slashItems = [
    ...
    },
    {
      ...
      command: (editor2) => editor2.chain().focus().toggleHeading({ level: 1 }).run()
    },
    {
      title: "Heading 2",
    ...
    },
    {
      ...
      command: (editor2) => editor2.chain().focus().toggleBulletList().run()
    },
    {
      title: "Checklist",
      description: "Track tasks with checkboxes",
      keywords: ["tasks", "todo", "checklist"],
      command: (editor2) => editor2.chain().focus().toggleTaskList().run()
    },
    {
      title: "Numbered List",
      description: "Create an ordered list",
      keywords: ["list", "ordered", "numbers"],
      command: (editor2) => editor2.chain().focus().toggleOrderedList().run()
    },
    {
      title: "Quote",
      description: "Insert a blockquote",
      keywords: ["blockquote", "quote", "citation"],
      command: (editor2) => editor2.chain().focus().toggleBlockquote().run()
    },
    {
      title: "Code Block",
      description: "Insert a code block",
      keywords: ["code", "snippet", "monospace"],
      command: (editor2) => editor2.chain().focus().toggleCodeBlock().run()
    },
    {
      title: "Table",
      description: "Insert a 3x3 table",
      keywords: ["table", "grid", "rows"],
      command: (editor2) => editor2.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()
    }
  ];
  var createSlashMenu = () => {
    let element = null;
        ...
        Suggestion({
          ...
          items: ({ query }) => {
            ...
            return slashItems.filter((item) => {
              return item.title.toLowerCase().includes(normalized) || item.description.toLowerCase().includes(normalized) || item.keywords.some((keyword) => keyword.includes(normalized));
            });
          },
          command: ({ editor: editor2, range, props }) => {
            editor2.chain().focus().deleteRange(range).run();
            props.command(editor2);
          },
    ...
    editor.chain().focus().insertContent([
      {
        ...
        attrs: {
          title: source.title,
          url: source.url,
          capturedAt: source.capturedAt ?? (/* @__PURE__ */ new Date()).toISOString()
        }
      },
      {
        type: "paragraph"
  ...
  window.NoteEditor = {
    receiveCommand(command2, payload) {
      try {
        switch (command2) {
          case "loadDocument":
